...
class GAT(torch.nn.Module):
    def __init__(self,categories_nums, features_num=16, num_class=2, sparse=False, edge_num=10000):
...
#        heads = 1
        ...
        if features_num>0:
            ...
            self.conv1 = GATConv(id_embed_size+embed_size*n+hidden, hidden, heads=heads, concat=True, dropout=dropout)
        else:
            self.ln0 = torch.nn.LayerNorm(id_embed_size+embed_size*n)
            self.conv1 = GATConv(id_embed_size+embed_size*n, hidden, heads=heads, concat=True, dropout=dropout)
...
class GAT_noLN(torch.nn.Module):
...
class GATModel:
    ...
    def init_model(self,model_data,table):
        ...
        if table.especial:
            model = GAT_noLN(categories_nums,features_num=data.x.size()[1], num_class=table.n_class,sparse=sparse)
        else:
            model = GAT(categories_nums,features_num=data.x.size()[1], num_class=table.n_class,sparse=sparse,edge_num=table.df_edge.shape[0])
        
        model = model.to(self.device)
        data = data.to(self.device)

#        optimizer = torch.optim.Adam(model.parameters(), lr=self.learning_rate,weight_decay=5e-4)
#        optimizer = torch.optim.SGD(model.parameters(), lr=0.2, momentum=0.9,weight_decay=0.01,dampening=0.1)
#        optimizer = torch.optim.Adagrad(model.parameters(), lr=0.1, lr_decay=0.1)
#        optimizer = torch.optim.RMSprop(model.parameters(), lr=0.01,alpha=0.9,momentum=0.1)
