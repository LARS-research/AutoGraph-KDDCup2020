...
from linear_stack import linear_stack
...
def fusion3(acc_list,run_model,run_type,pred_matrix_list,valid_pred_test_matrixs_list,valid_matrix_list,valid_label,test_label,degree_mean,has_feature):
    weights = cal_wei(acc_list,degree_mean,has_feature=has_feature)
    
    
    n = len(valid_matrix_list)
    stack_model = linear_stack(n, weights.tolist(), iters = 100, rate_max = 0.25, rate_min = 0.01, verbose=1)
    stack_model.fit(np.array(valid_matrix_list),valid_label)
    
    LOGGER.info(f'model_weights1:{weights}')
    LOGGER.info(f'model_weights:{stack_model.W}')
    
    preds = stack_model.predict(np.array(valid_pred_test_matrixs_list))
    valid = stack_model.predict(np.array(valid_matrix_list))
...
#    if table.unbalanced:
#        LOGGER.info('weight 500')
#        weights = torch.nn.functional.softmax( torch.Tensor(weight_list)*500, dim=0 ).numpy()        
#    elif table.especial:
#        LOGGER.info('weight 30')
#        weights = torch.nn.functional.softmax( torch.Tensor(weight_list)*30, dim=0 ).numpy()        
#    else:
#        LOGGER.info('weight 100')
#        weights = torch.nn.functional.softmax( torch.Tensor(weight_list)*100, dim=0 ).numpy()
            
    
#    weight_rate = np.array(acc_list)
#    weight_min = weight_rate.min()
#    if weight_rate.shape[0]>=3:
#        weight_min = np.partition(weight_rate,2)[2]
#        
#    weight_rate = (10/(weight_rate.max()-weight_min+1e-5)+3)**(3/4)
#    weight_rate = min(500,weight_rate)#min(200,weight_rate)
#    LOGGER.info(f'weight rate:{weight_rate}')
#    weights = torch.nn.functional.softmax( torch.Tensor(acc_list)*weight_rate, dim=0 ).numpy()
...
class Model:
    ...
    def train_predict(self, data, time_budget,n_class,schema):
        ...
        feat = Feat()
#        feat.fit_transform_first(table)
...
#        #drop lgb ori_columns
#        if table.ori_columns.shape[0]<1500:
#            process_data.drop_ori_columns(table)
            
        #gcn,sage,gat,agnn,dna
#        model_type_list = ['gat','gcn','lgb','tagc','arma','sage','agnn']
        
#        if table.especial:
#            model_type_list = model_type_list+['sage']*10+['gat']*2
...
#        if table.undirected_graph and not table.sparse:
##        if True:
#            categories.append('subgraph')
