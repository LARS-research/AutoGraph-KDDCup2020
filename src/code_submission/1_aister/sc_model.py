...
class SC(torch.nn.Module):
    ...
    def __init__(self,categories_nums, features_num=16, num_class=2, sparse=False):
        ...
        self.conv1 = SplineConv(embed_size*len(categories_nums)+features_num, hidden, dim=3, kernel_size=4)
        self.conv2 = SplineConv(hidden,num_class, dim=3, kernel_size=4)
    ...
    def forward(self, data):
        ...
        x = F.elu(self.conv1(x, edge_index,target_degree))
        x = F.dropout(x, p=self.dropout_p,training=self.training)
        x = self.conv2(x, edge_index,target_degree)
...
class SCModel:
    ...
    def init_model(self,model_data,table):    
        data = model_data.sc_data
        sparse = table.sparse
        categories_nums = data.categories_nums
        model = SC(categories_nums,features_num=data.x.size()[1], num_class=table.n_class,sparse=sparse)
