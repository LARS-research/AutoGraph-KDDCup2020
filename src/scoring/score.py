...
def logging_score(args):
    y_true = _get_solution(args.solution_dir)['label']
    y_pred = _get_prediction(args.prediction_dir)
    acc = accuracy_score(y_true, y_pred)
    bacc = balanced_accuracy_score(y_true, y_pred)

    if args.solution_dir[-1] != '/':
        dataname = args.solution_dir[-1]
    else:
        dataname = args.solution_dir[-2]
    if args.code_dir[-1] != '/':
        methodname = args.code_dir.split('/')[-1]
    else:
        methodname = args.code_dir[:-1].split('/')[-1]
    timestamp = datetime.datetime.today().strftime('%Y%m%d%H%M%S')

    filename = '_'.join(['log', methodname, dataname, timestamp])

    with open('logs/'+filename, 'w') as ftmp:
        ftmp.write(f'acc:{acc:.4f}, bacc:{bacc:.4f}')
...
def _finalize(score, scoring_start):
    ...
    LOGGER.info(
        ...
        f"Scoring duration: {duration:.2f} sec. "
        f"The score of your algorithm on the task is: {score:.4f}.")
